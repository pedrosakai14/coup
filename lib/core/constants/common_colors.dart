import 'dart:ui';

class CommonColors {
  const CommonColors._();

  static Color get primaryColor => Color(0xFF5e2842);
  static Color get secondaryColor => Color(0xFF299f6a);
  static Color get tertiaryColor => Color(0xDD000000);

  static Color get errorColor => Color(0xFFc42c2c);

  static Color get backgroundPrimaryColor => Color(0xFF5d2842);
  static Color get backgroundDisabledColor => Color(0xFFadadad);
  static Color get backgroundPrimaryCardColor => Color(0xFFd1dae4);
  static Color get backgroundSecondaryCardColor => Color(0xFFEEEEEE);

  static Color get black => Color(0xFF151515);
  static Color get white => Color(0xFFFFFFFF);
  static Color get grey => Color(0xFF9E9E9E);
}